...
class ChatDebouncer:
    ...
    def add(self, update: BufferedUpdate) -> None:
        ...
        with self._lock:
            ...
            if old is not None:
                old.cancel()
            timer = threading.Timer(self._delay_s, self._flush_safe, args=(chat_id,))
            timer.daemon = True
    ...
    def flush_now(self, chat_id: str) -> list[BufferedUpdate]:
        ...
        if items:
            logger.info(
                "debounce_flush chat_id=%s n=%s texts=%s",
                chat_id,
                len(items),
...
def merge_buffered_images(updates: list[BufferedUpdate]) -> str:
    """Prefer the last non-empty image data URL in the turn."""
    last = ""
    for u in updates:
